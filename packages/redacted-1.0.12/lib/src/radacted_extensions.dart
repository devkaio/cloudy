        ...
        for (var box in boxes) {
          children.add(
            _RedactedFillWidget(
              ...
              child: Container(
                margin: boxes.indexOf(box) != boxes.length ? EdgeInsets.only(bottom: boxes.isNotEmpty && boxes.indexOf(box) - 1 != boxes.length ? 2 : 0) : null,
                width: box.right,
...
class _MeasuredWidgetState extends State<MeasuredWidget> {
  ...
  Widget build(BuildContext context) {
    return MeasureSize(
      ...
      child: newChild == null ? Opacity(opacity: 0, child: widget.child) : _RedactedFillWidget(configuration: widget.configuration, child: newChild!),
    );
  }
}
